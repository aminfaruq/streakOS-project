...
// MARK: - Timer UI
struct IOSTimerItemCardView: View {
    let progress: ItemProgress
    var isEditMode: Bool
    let onToggleTimer: () -> Void
    let onTap: () -> Void
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(timerDisplayText(liveNow: liveNow))
                        ...
                        .foregroundStyle(isTimerRunning ? IOSDesignTokens.accent : .secondary)
                        .padding(.horizontal, 8)
                        .padding(.vertical, 4)
    ...
    private var timerButton: some View {
        Button(action: onToggleTimer) {
            Image(systemName: isTimerRunning ? "pause.fill" : "play.fill")
                .font(.system(size: 20))
                .frame(width: 48, height: 48)
                .foregroundStyle(isTimerRunning ? .white : IOSDesignTokens.accent)
                .background(
                    Circle()
                        .fill(isTimerRunning ? IOSDesignTokens.accent : IOSDesignTokens.accent.opacity(0.1))
                )
        }
        .buttonStyle(.plain)
    }
